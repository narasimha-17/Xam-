...
async def start_attempt(exam_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user)):
    ...
    if user.role != UserRole.admin and not (
        user.roll_number and user.section and user.department and user.gender and user.education_level
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Complete your profile (roll number, section, department, gender, and education level) "
