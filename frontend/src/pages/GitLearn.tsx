...
import { CheckCircle2, GitBranch, Lightbulb, Lock, RotateCcw, Terminal, Trophy } from "lucide-react";
import { GIT_LEVELS, buildScenario } from "../lib/gitLevels";
import { runCommand, type RepoState } from "../lib/gitSim";
import { completeGitLevel, fetchGitProgress } from "../lib/gitLearnApi";
...
function tierFor(xp: number): { label: string; color: string } {
  if (xp >= 140) return { label: "Git Master", color: "text-accent" };
...
export function GitLearn() {
  const queryClient = useQueryClient();
  const { data: progress, isLoading } = useQuery({
    queryKey: ["git-progress"],
    queryFn: fetchGitProgress,
  });

  const [levelIndex, setLevelIndex] = useState(0);
  const [hasPickedStart, setHasPickedStart] = useState(false);
  const [repoState, setRepoState] = useState<RepoState>(() => buildScenario(GIT_LEVELS[0].setup));
  ...
  useEffect(() => {
    ...
    const firstIncomplete = GIT_LEVELS.findIndex((l) => !completed.has(l.id));
    setLevelIndex(firstIncomplete === -1 ? GIT_LEVELS.length - 1 : firstIncomplete);
    setHasPickedStart(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [progress]);

  useEffect(() => {
    setRepoState(buildScenario(level.setup));
    setLog([]);
    setShowHint(false);
  ...
  const completeMutation = useMutation({
    mutationFn: () => completeGitLevel(level.id, level.xp),
    onSuccess: (data) => {
      queryClient.setQueryData(["git-progress"], data);
    },
  });

  function isUnlocked(index: number): boolean {
    if (index === 0) return true;
    return completed.has(GIT_LEVELS[index - 1].id) || completed.has(GIT_LEVELS[index].id);
  }

  function runInput() {
    const cmd = input.trim();
    if (!cmd) return;
    const result = runCommand(repoState, cmd);
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-2 font-display text-2xl font-semibold text-ink">
            <GitBranch size={24} className="text-accent-soft" /> Git &amp; GitHub practice
          </h1>
          <p className="mt-1 text-sm text-ink-muted">
            Type real git commands in a simulated repo and watch the commit graph update live.
          </p>
        </div>
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">
                  Level {levelIndex + 1} of {GIT_LEVELS.length} &middot; {level.xp} XP
                </p>
