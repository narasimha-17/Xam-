...
import { CheckCircle2, Lightbulb, Lock, RotateCcw, Ship, Terminal, Trophy } from "lucide-react";
import { K8S_LEVELS, buildK8sScenario } from "../lib/k8sLevels";
import { runKubectlCommand, type K8sState } from "../lib/k8sSim";
import { completeK8sLevel, fetchK8sProgress } from "../lib/k8sLearnApi";
import { K8sViz } from "../components/k8s/K8sViz";
...
function tierFor(xp: number): { label: string; color: string } {
  if (xp >= 120) return { label: "Cluster Captain", color: "text-accent" };
  if (xp >= 75) return { label: "Gold", color: "text-warning" };
  if (xp >= 35) return { label: "Silver", color: "text-ink-muted" };
  return { label: "Bronze", color: "text-accent-dim" };
}

export function K8sLearn() {
  const queryClient = useQueryClient();
  const { data: progress, isLoading } = useQuery({
    queryKey: ["k8s-progress"],
    queryFn: fetchK8sProgress,
  });

  const [levelIndex, setLevelIndex] = useState(0);
  const [hasPickedStart, setHasPickedStart] = useState(false);
  const [k8sState, setK8sState] = useState<K8sState>(() => buildK8sScenario(K8S_LEVELS[0].setup));
  ...
  const level = K8S_LEVELS[levelIndex];

  useEffect(() => {
    if (!progress || hasPickedStart) return;
    const firstIncomplete = K8S_LEVELS.findIndex((l) => !completed.has(l.id));
    setLevelIndex(firstIncomplete === -1 ? K8S_LEVELS.length - 1 : firstIncomplete);
    setHasPickedStart(true);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [progress]);

  useEffect(() => {
    setK8sState(buildK8sScenario(level.setup));
    setLog([]);
    setShowHint(false);
  ...
  const completeMutation = useMutation({
    mutationFn: () => completeK8sLevel(level.id, level.xp),
    onSuccess: (data) => {
      queryClient.setQueryData(["k8s-progress"], data);
    },
  });

  function isUnlocked(index: number): boolean {
    if (index === 0) return true;
    return completed.has(K8S_LEVELS[index - 1].id) || completed.has(K8S_LEVELS[index].id);
  }

  function runInput() {
    const cmd = input.trim();
    if (!cmd) return;
    const result = runKubectlCommand(k8sState, cmd);
    setLog((prev) => [...prev, { command: cmd, output: result.output, error: result.error }]);
    setK8sState(result.state);
    setInput("");
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="flex items-center gap-2 font-display text-2xl font-semibold text-ink">
            <Ship size={24} className="text-accent-soft" /> Kubernetes practice
          </h1>
          <p className="mt-1 text-sm text-ink-muted">
            Type real kubectl commands against a simulated cluster and watch pods, deployments, and services update
            live.
          </p>
        </div>
        ...
        <div className="flex flex-1 flex-col gap-4">
            ...
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-medium uppercase tracking-wide text-ink-faint">
                  Level {levelIndex + 1} of {K8S_LEVELS.length} &middot; {level.xp} XP
                </p>
          ...
          <div className="flex flex-col gap-2 rounded-2xl bg-ink p-4 shadow-glow">
            ...
            <div className="max-h-56 overflow-y-auto rounded-lg bg-black/20 p-3 font-mono text-xs leading-relaxed text-base">
              {log.length === 0 && <p className="text-base/40">Type a kubectl command below and press Enter.</p>}
              {log.map((entry, i) => (
